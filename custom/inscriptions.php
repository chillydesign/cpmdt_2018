    ...
    if (in_array($course_type,  array('14musicale', '47musicale', 'instrumentchant'))) :
       ...
       <div id="other_places_container"></div>
        ...
        $rq_frm .= '<script id="multiloc_template" type="x-underscore">
         <%  _.each(locations,function(location,key,list){  %>
        <label> <input class="checkbox_input" type="checkbox" name="other_place_possible_ids[]" value="<%= location.wid %>" /> <%= location.post_title %> </label><br>
         <% }) %>
         </script>';
...
function add_download_link_inscription($which) {
    if (is_post_type_archive('inscription')) {
        ...
        echo download_insc_button('adults', 'adultes');
